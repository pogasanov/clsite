...
class ProfileViewSetTest(APITestCase):
    ...
    def test_partial_update(self):
        ...
        response = self.client.patch(self.VIEW_URL, {"summary": NEW_SUMMARY})
        self.assertEqual(response.status_code, 200)
        self.user.refresh_from_db()
        self.assertEqual(NEW_SUMMARY, self.user.summary)
    ...
    def test_update_doesnt_delete_address(self):
        RANDOM_PAYLOAD = {"summary": "Test"}
        self.assertIsNotNone(self.user.address)

        response = self.client.patch(self.VIEW_URL, RANDOM_PAYLOAD)
        self.assertEqual(response.status_code, 200)
        self.user.refresh_from_db()
        self.assertIsNotNone(self.user.address)
    ...
    def test_update_and_add_language_data(self):
        ...
        UPDATED_LANGUAGE_PAYLOAD = [
            {"id": existing_languages[0].id, "name": "zh", "proficiency_level": "NS"},
            {"id": existing_languages[1].id, "name": "is", "proficiency_level": "CF"},
            {"name": "de", "proficiency_level": "PF"},
            {"name": "fj", "proficiency_level": "CF"},
        ]

        response = self.client.patch(self.VIEW_URL, {"languages": UPDATED_LANGUAGE_PAYLOAD})
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_and_add_education_data(self):
        ...
        UPDATED_EDUCATION_PAYLOAD = [
            ...
            {"school": "Dummy school3", "degree": "Dummy degree3", "graduation_date": "2012-10-16"},
            {"school": "Dummy school4", "degree": "Dummy degree4", "graduation_date": "2014-10-16"},
        ]

        response = self.client.patch(self.VIEW_URL, {"educations": UPDATED_EDUCATION_PAYLOAD})
        self.assertEqual(response.status_code, 200)
        ...
        UPDATED_WORK_EXPERIENCE_PAYLOAD = [
            {
                ...
                "duration": {"lower": "2010-6-16", "upper": "2010-10-16"},
            },
            {
                "id": existing_workexperiences[1].id,
                "company_name": "Dummy company2",
                "position": "Dummy position2",
                "duration": {"lower": "2011-6-16", "upper": "2011-10-16"},
            },
            {
                "company_name": "Dummy company3",
                "position": "Dummy position3",
                "duration": {"lower": "2012-6-16", "upper": "2012-10-16"},
            },
            {
                "company_name": "Dummy company4",
                "position": "Dummy position4",
                "duration": {"lower": "2013-6-16", "upper": "2013-10-16"},
            },
        ]

        response = self.client.patch(self.VIEW_URL, {"work_experiences": UPDATED_WORK_EXPERIENCE_PAYLOAD})
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_and_add_organization_data(self):
        ...
        response = self.client.patch(self.VIEW_URL, {"organizations": UPDATED_ORGANIZATION_PAYLOAD})
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_and_add_award_data(self):
        ...
        UPDATED_AWARDS_PAYLOAD = [
            ...
            {"title": "Dummy title3", "presented_by": "Dummy name3", "year": "2012"},
            {"title": "Dummy title4", "presented_by": "Dummy name4", "year": "2013"},
        ]

        response = self.client.patch(self.VIEW_URL, {"awards": UPDATED_AWARDS_PAYLOAD})
        self.assertEqual(response.status_code, 200)
