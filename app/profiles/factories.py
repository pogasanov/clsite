...
from profiles.utils import (
    LAW_TYPE_TAGS_CHOICES,
    SUBJECTIVE_TAGS_CHOICES,
    COUNTRIES_CHOICES,
    _get_states_for_country,
)
...
class AddressFactory(factory.django.DjangoModelFactory):
    ...
    state = factory.LazyAttribute(
        lambda o: random.choice(_get_states_for_country(o.country))[0]
        if _get_states_for_country(o.country)
        else None
    )
    city = factory.LazyAttribute(lambda o: factory.Faker("city").generate() if o.state else None)
...
class ProfileFactory(factory.django.DjangoModelFactory):
    ...
    email_confirmed_at = factory.Faker(
        "date_time_between", start_date="-3y", end_date="now", tzinfo=timezone.utc
    )
    ...
    bio = factory.LazyFunction(
        lambda: "<p>" + "</p><p>".join(factory.Faker("paragraphs", nb=3).generate()) + "</p>"
        if random.random() < 0.5
        else ""
    )
    experience = factory.LazyFunction(
        lambda: factory.Faker("pyint", min_value=0, max_value=30, step=1).generate()
        if random.random() < 0.5
        else ""
    )
    ...
    size_of_clients = factory.LazyFunction(
        lambda: factory.Faker("pyint", min_value=0, max_value=3, step=1).generate()
        if random.random() < 0.5
        else None
    )
    ...
    subjective_tags = factory.LazyFunction(
        lambda: [
            get_random_subjective_tag()
            for _ in range(random_number_exponential_delay(pr=0.25, probability_of_none=0.0))
        ]
    )
    summary = factory.LazyFunction(
        lambda: factory.Faker("catch_phrase").generate() if random.random() < 0.5 else ""
    )
    ...
    jurisdiction = factory.RelatedFactoryList(
        JurisdictionFactory, "profile", size=lambda: random.randrange(3) + 1
    )
    ...
    workexperience = factory.RelatedFactoryList(
        WorkExperienceFactory, "profile", size=lambda: random.randrange(3)
    )
    organization = factory.RelatedFactoryList(
        OrganizationFactory, "profile", size=lambda: random.randrange(3)
    )
    ...
    class Params:
        ...
        no_attorney_proof = factory.Trait(
            email_not_confirmed=True, passport_photo=None, bar_license_photo=None
        )

        email_not_confirmed = factory.Trait(email_confirmed_at=None)
